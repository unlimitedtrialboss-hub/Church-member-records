...
import {
  ...
  getListNotionDatabasesQueryKey,
  useArchiveMember,
  useCreateMember,
  useGetMember,
  useGetMemberSummary,
  ...
  return <div className="grain min-h-[100dvh] bg-background">
    ...
    <main className="min-h-[100dvh] md:pl-[248px]">
      <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-border/70 bg-background/90 px-5 backdrop-blur-md md:px-10">
        ...
        <div className="hidden text-xs font-semibold text-muted-foreground sm:block"><span className="text-primary">IHBC</span><span className="mx-2 opacity-40">/</span>{selectedDatabase?.title || 'Choose a member registry'}</div>
        <div className="ml-auto flex items-center gap-2"><div className="hidden items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground sm:flex"><span className="size-1.5 rounded-full bg-primary" />Private workspace</div>{selectedDatabase && <button onClick={onClearDatabase} data-testid="button-switch-database" className="rounded-lg px-3 py-2 text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">Switch registry</button>}</div>
      </header>
...
function DatabasePicker({ databases, isLoading, onSelect, error }: { databases?: Database[]; isLoading: boolean; onSelect: (db: Database) => void; error: boolean }) {
  ...
  const filtered = (databases ?? []).filter((db) => db.title.toLowerCase().includes(query.toLowerCase()));
  return <div className="mx-auto max-w-3xl animate-rise">
    <div className="mb-10 max-w-xl"><Badge tone="sand">First, choose your registry</Badge><h1 className="mt-4 font-display text-5xl leading-[.98] tracking-tight text-primary md:text-6xl">A good record<br /><em className="font-normal text-accent">keeps a story.</em></h1><p className="mt-5 max-w-md text-base leading-relaxed text-muted-foreground">Connect a Notion database to begin caring for the people who make up International Heritage Baptist Church.</p></div>
    <div className="rounded-2xl border border-border bg-card p-4 shadow-[0_16px_45px_rgba(38,69,61,.06)] md:p-6">
      <div className="mb-5 flex items-center justify-between gap-3"><div><h2 className="font-display text-2xl">Your member registries</h2><p className="mt-1 text-xs text-muted-foreground">Only databases shared with this workspace appear here.</p></div><Landmark className="text-accent" size={23} /></div>
...
function Home() {
  const [selectedDatabase, setSelectedDatabase] = useState<Database | undefined>(() => { try { const stored = localStorage.getItem('ihbc-database'); return stored ? JSON.parse(stored) : undefined; } catch { return undefined; } });
  const [search, setSearch] = useState('');
  const databasesQuery = useListNotionDatabases(undefined, { query: { queryKey: getListNotionDatabasesQueryKey(), staleTime: 30_000 } });
  ...
  const choose = (database: Database) => { setSelectedDatabase(database); localStorage.setItem('ihbc-database', JSON.stringify(database)); };
  if (!selectedDatabase) return <Shell onClearDatabase={() => undefined}><DatabasePicker databases={databasesQuery.data} isLoading={databasesQuery.isLoading} error={databasesQuery.isError} onSelect={choose} /></Shell>;
  const stats = summaryQuery.data;
  return <Shell selectedDatabase={selectedDatabase} onClearDatabase={() => { setSelectedDatabase(undefined); localStorage.removeItem('ihbc-database'); }}>
    <div className="animate-rise">
      ...
      <p className="mt-5 text-center text-[11px] text-muted-foreground">Last synced with Notion {displayDate(selectedDatabase.lastEditedTime)}</p>
    </div>
  </Shell>;
}
  ...
  return <form onSubmit={submit} className="animate-rise">
    ...
    <div className="rounded-2xl border border-border bg-card p-5 shadow-[0_10px_35px_rgba(38,69,61,.04)] md:p-8">
     ...
     <div className="mt-7 flex flex-col-reverse gap-3 border-t border-border pt-5 sm:flex-row sm:items-center sm:justify-between">
       <p className="text-xs text-muted-foreground">When you are finished, submit this record to save it in Notion.</p>
...
function NewMember({ database }: { database?: Database }) {
  const [, setLocation] = useLocation();
  const clearDatabase = () => { localStorage.removeItem('ihbc-database'); setLocation('/'); };
  if (!database) return <ChooseDatabaseNotice />;
  return <Shell selectedDatabase={database} onClearDatabase={clearDatabase}><MemberForm database={database} /></Shell>;
}

function ChooseDatabaseNotice() {
  return <Shell onClearDatabase={() => undefined}><div className="mx-auto max-w-xl rounded-2xl border border-border bg-card p-10 text-center"><div className="mx-auto grid size-14 place-items-center rounded-2xl bg-secondary text-secondary-foreground"><Landmark size={25} /></div><h1 className="mt-5 font-display text-3xl">Choose a registry first</h1><p className="mt-2 text-sm leading-relaxed text-muted-foreground">Select your church’s Notion database from the directory before creating a member record.</p><Link href="/" data-testid="link-choose-database" className="mt-6 inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground">Go to directory <ArrowLeft className="rotate-180" size={15} /></Link></div></Shell>;
}
...
function MemberDetail({ database }: { database?: Database }) {
  ...
  const [, setLocation] = useLocation();
  const clearDatabase = () => { localStorage.removeItem('ihbc-database'); setLocation('/'); };
  const memberQuery = useGetMember(params.id, { databaseId: database?.id ?? '' }, { query: { queryKey: getGetMemberQueryKey(params.id, { databaseId: database?.id ?? '' }), enabled: Boolean(database?.id && params.id) } });
  if (!database) return <ChooseDatabaseNotice />;
  if (memberQuery.isLoading) return <Shell selectedDatabase={database} onClearDatabase={clearDatabase}><div className="grid gap-4"><div className="skeleton h-32 rounded-2xl" /><div className="skeleton h-[500px] rounded-2xl" /></div></Shell>;
  if (memberQuery.isError || !memberQuery.data) return <Shell selectedDatabase={database} onClearDatabase={clearDatabase}><div className="py-20 text-center"><X className="mx-auto text-destructive" size={28} /><h1 className="mt-4 font-display text-3xl">Record unavailable</h1><Link href="/" data-testid="link-return-directory" className="mt-5 inline-flex text-sm font-semibold text-primary">Return to directory</Link></div></Shell>;
  return <Shell selectedDatabase={database} onClearDatabase={clearDatabase}><MemberForm memberId={params.id} database={database} initial={memberQuery.data} /></Shell>;
}

function AppRouter() {
  const [currentLocation] = useLocation();
  const selectedDatabase = useMemo<Database | undefined>(() => { try { const stored = localStorage.getItem('ihbc-database'); return stored ? JSON.parse(stored) : undefined; } catch { return undefined; } }, [currentLocation]);
  return <ErrorBoundary resetKey={currentLocation}><Switch><Route path="/" component={Home} /><Route path="/members/new"><NewMember database={selectedDatabase} /></Route><Route path="/members/:id"><MemberDetail database={selectedDatabase} /></Route><Route component={NotFound} /></Switch></ErrorBoundary>;
}
